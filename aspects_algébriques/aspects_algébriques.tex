...
Commençons par un exemple. Nous définissons la fonction $f(x,y,z)=e^{(x^2-y^2)z}$. Son graphe de calcul est représenté en Figure \ref{fig:ex_graphe_calcul}.
...
\begin{tikzpicture}[node distance=15cm]
    ...
    \node[draw, circle] (x2) at (3, 0) {$a=x^2$};
    ...
    % Arrows
    \draw[->] (x) -- node[midway,above] {$t \mapsto t^2$} (x2);
